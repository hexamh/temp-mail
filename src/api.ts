...
async function getEmail(token: string, emailId: string, env: Env): Promise<Response> {
  ...
  // Ensure this domain matches your actual deployed worker domain
  const baseUrl = `https://tempmail.drkingbd.cc`; 
...
async function getAttachment(
  ...
  // Apply CORS headers to attachments
  Object.entries(CORS_HEADERS).forEach(([k, v]) => headers.set(k, v));

  const response = new Response(object.body, { headers });

  // 4. Store in Cache for future requests asynchronously
